...
predict2_template_name = 'predict2.html'
...
ALLOWED_VIDEO_EXTENSIONS = set(['mp4','webm','avi','3gp','wmv','flv','mkv'])
    ...
    if request.method == 'GET':
        ...
        if video_upload_form.is_valid():
            ...
            if allowed_video_file(video_file.name) == False:
                if allowed_image_file(video_file.name) == False:
                    video_upload_form.add_error("upload_video_file","This file is not allowed ")
                    return render(request, index_template_name, {"form": video_upload_form})
                else:
                    saved_image_file = 'uploaded_file_'+str(int(time.time()))+"."+video_file_ext
                    with open(os.path.join(settings.PROJECT_DIR, 'uploaded_images', saved_image_file), 'wb') as vFile:
                        shutil.copyfileobj(video_file, vFile)

                    request.session['file_name'] = os.path.join(settings.PROJECT_DIR, 'uploaded_images', saved_image_file)
                    request.session['sequence_length'] = sequence_length
                    return redirect('ml_app:predict2')
...
def predict2_page(request):
    if request.method == "GET":
        if 'file_name' not in request.session:
            return redirect("ml_app:home")
        if 'file_name' in request.session:
            image_file = request.session['file_name']
        uploaded_image = image.load_img(image_file)
        ...
        probab = model.predict(np_image)[0][0] * 100
        pcnt = round(probab,2)


    return render(request, predict2_template_name,{'result': pcnt})
